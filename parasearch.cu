...
extern __global__ void deviceRT(volatile int *done, volatile int *totalExecTasks, struct kernel_para_GPU *warpPool, volatile struct kernel_para *taskBuffer, struct kernel_para *taskArgs, volatile int *exec, volatile int *totalScheTasksDev);
...
//		if(i == tasks-1) printf("Host:%p\n", taskArgs[i].A);

        }

	for(int i = 0; i < (BSize*SSize); i++){
		taskparaBuffer[i].req = 0;
		taskparaBuffer[i].ready = 0;
	}
	...
	*exec = 0;
	...
	//cudaFuncSetCacheConfig(deviceRT, cudaFuncCachePreferShared);
	...
	startTime = my_timer();
	while(j < tasks){
		for(i = 0; i < (SSize*BSize); i++){
			if(taskparaBuffer[i].req == 0 && (i%SBuf) != 0){
				taskparaBuffer[i].warp = THREADS/32;
				taskparaBuffer[i].req = 1;
				taskparaBuffer[i].ready = 1;
				taskparaBuffer[i].taskId = j;
				//if(j == (tasks-1))printf("Host:%d, %d, %d\n", i, j, SBuf);
